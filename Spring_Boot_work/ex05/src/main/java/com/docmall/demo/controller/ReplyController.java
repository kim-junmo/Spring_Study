...
public class ReplyController {
	...
	//클라이언트의 요청주소: 1) /replies/pages/100/1 REST API기술이론에서 권장하는 주소형태, 2) /replies?pages=1&bno=100 전통적인 주소
	///replies/pages/100/1 주소(경로)에 존재하는 값을 사용할 때 구분되는 위치에 {이름}을 사용해야 한다.
	...
	public ResponseEntity<Map<String, Object>> getList(@PathVariable("bno")Long bno, @PathVariable("page") int page) {
		...
		// 1)댓글 목록 작업
		Criteria cri = new Criteria(1, 10); //댓글 목록에 첫 패이지, 페이지마다 10개씩 보여주겠다는 뜻.
		List<ReplyVO> list = replyService.getListPaging(cri, bno);
		map.put("list", list);
		
		//2) 댓글페이징 작업
		int total = replyService.getCountByBno(bno);
		pageDTO pageDTO = new pageDTO(cri, page); 
